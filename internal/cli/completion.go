...
func completion(args []string, stdout io.Writer) error {
  ...
  if [[ "${COMP_WORDS[1]}" == "host" && "${COMP_WORDS[2]}" == "ls" ]]; then
    ...
    COMPREPLY=($(compgen -W "host ls tail continue doctor update completion version help" -- "$current"))
  fi
}
...
_arguments \
  '1:command:(host ls tail continue doctor update completion version help)' \
  '2:host action:(ls)'`)
	case "fish":
